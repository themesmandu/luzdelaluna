 ...
 */
function luzdelaluna_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'luzdelaluna_pingback_header' );
 ...
 * @return array
 */
function luzdelaluna_body_classes( $classes ) {
...
add_filter( 'body_class', 'luzdelaluna_body_classes' );
 ...
 * @return array
 */
function luzdelaluna_post_classes( $classes ) {
...
add_action( 'post_class', 'luzdelaluna_post_classes' );
 ...
 */
function luzdelaluna_excerpt_more( $link ) {
	if ( is_admin() ) {
		return $link;
	}
...
add_filter( 'excerpt_more', 'luzdelaluna_excerpt_more' );
add_filter( 'the_content_more_link', 'luzdelaluna_excerpt_more' );
 ...
 */
function luzdelaluna_bootstrap_class_images( $html ) {
...
add_filter( 'the_content', 'luzdelaluna_bootstrap_class_images', 10 );
 ...
 */
function luzdelaluna_bootstrap_table_class( $content ) {
	return str_replace( '<table', '<table class="table"', $content );
}
add_filter( 'the_content', 'luzdelaluna_bootstrap_table_class' );

/**
 * Adds a class to the navigation links of posts
 */
function luzdelaluna_posts_link_attributes() {
 ...
 */
function luzdelaluna_custom_excerpt_length() {
	return 16;
}
add_filter( 'excerpt_length', 'luzdelaluna_custom_excerpt_length' );
 ...
 */
function luzdelaluna_front_page( $template ) {
	return is_home() ? '' : $template;
}
add_filter( 'frontpage_template', 'luzdelaluna_front_page' );
