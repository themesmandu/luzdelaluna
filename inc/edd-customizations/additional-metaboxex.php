	...
	foreach ( $gallery_metabox['fields'] as $field ) {
		...
		switch ( $field['type'] ) {
			case 'upload':
				...
				<p>
				<input type="url" style="max-width:550px; width:100%" name="<?php echo esc_html( $field['id'] ); ?>" id="<?php echo esc_html( $field['id'] ); ?>" value="<?php echo esc_url( $meta ? $meta : '' ); ?>" placeholder="Upload or enter the image URL">
